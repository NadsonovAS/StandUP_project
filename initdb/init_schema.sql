...
CREATE TABLE IF NOT EXISTS standup_core.llm_chapters (
    ...
    FOREIGN KEY (video_id, start_segment_id)
        REFERENCES standup_core.transcript_segments (video_id, segment_id)
        ON DELETE CASCADE,
    FOREIGN KEY (video_id, end_segment_id)
        REFERENCES standup_core.transcript_segments (video_id, segment_id)
        ON DELETE CASCADE
);

CREATE TABLE IF NOT EXISTS standup_core.llm_categories (
    category_id   INTEGER PRIMARY KEY,
    main_category TEXT UNIQUE NOT NULL
);

CREATE TABLE IF NOT EXISTS standup_core.llm_subcategories (
...
CREATE TABLE IF NOT EXISTS standup_core.llm_classifications (
