# infer with commit id 5d5e0327916959d832d95ffbef5f484efc93d799 of https://github.com/jrgillick/laughter-detection
...
def get_audio_file_path(referenced_paper, audio_dir, audio_id):
    if referenced_paper == "McCowan2005":
        ...
        if os.path.exists(os.path.join(audio_dir, audio_id + ".opus")):
...
def main(referenced_paper):
    ...
    laughter_detection_repo = os.path.join(
        os.path.dirname(__file__), "..", "..", "..", "laughter-detection"
    )

    repo = git.Repo(laughter_detection_repo)
    if not repo.head.object.hexsha == "5d5e0327916959d832d95ffbef5f484efc93d799":
        raise ValueError(
            "Commit ID isn't equal to original version. This may cause unexpected result."
        )
        exit()
    ...
    for audio_id in evaluation_list:
        ...
        subprocess.run(
            [
                ...
                "&",
                sys.executable,
                os.path.join(laughter_detection_repo, "segment_laughter.py"),
                "--input_audio_file",
                audio_file_path,
                "--output_dir",
                out_dir,
