...
def compute_frame_wise_metrics(eval_laughter, gt_laughter):
    ...
    metrics = precision_recall_fscore_support(
        gt_laughter, eval_laughter, zero_division=0
    )
    ...
    if len(metrics[3]) == 1:  # only one class (all the values are the same)
        if gt_laughter[0] == 0:
            ...
            frame_wise_metrics["accuracy"].append(1.0)
            frame_wise_metrics["precision"].append(1.0)
            frame_wise_metrics["recall"].append(1.0)
            frame_wise_metrics["f1"].append(1.0)
        else:
            raise ValueError("Unknown gt_laughter[0]: {}".format(gt_laughter[0]))
    else:
        frame_wise_metrics["accuracy"].append(
            (gt_laughter == eval_laughter).sum() / len(gt_laughter)
        )
...
def compute_time_diff(eval_laughter_dict, gt_laugh_segment, audio_length):
    closest_values = sorted(
        eval_laughter_dict.values(),
        key=lambda x: abs(x["start_sec"] - gt_laugh_segment["start_sec"]),
    )
    ...
    assert 0 <= eval_start_time and eval_end_time <= audio_length + 1.0, (
        f"should {eval_start_time} <= {eval_end_time} <= {audio_length}"
    )
...
def main(dataset_name, neg_sample_scale=4):
    ...
    audio_dir = os.path.join(
        os.path.dirname(__file__), "datasets", dataset_name, "audio"
    )
    model_dir = os.path.join(os.path.dirname(__file__), "models")

    audio_length_cache = {}
    cache_file = os.path.join(
        os.path.dirname(__file__), "cache", dataset_name + ".json"
    )
    ...
    for model_name in os.listdir(model_dir):  # ["ours"]:
        ...
        assert len(glob.glob(eval_dir + "/*.json")), (
            f"No eval files found in {eval_dir}"
        )
        ...
        for gt_file in gt_files:
            ...
            with open(gt_file, "r", encoding="utf-8") as f:
                ...
                gt_laughter_dict = {
                    k: v
                    for k, v in gt_laughter_dict.items()
                    if not should_skip_sample(v)
                }
            ...
            if dataset_name in datasets_by_dynamic_generation:
                for gt_laugh_segment in gt_laughter_dict.values():
                    ...
                    if (
                        np.sum(
                            eval_laughter[
                                int(gt_start_time * hz) : int(gt_end_time * hz)
                            ]
                        )
                        >= 1
                    ):
                        TP += 1
                        detection_eval.append(1)
                        detection_gt.append(1)
                        compute_frame_wise_metrics(
                            eval_laughter[
                                int(gt_start_time * hz) : int(gt_end_time * hz)
                            ],
                            gt_laughter[
                        ...
                        tp_s, tp_e = compute_time_diff(
                            eval_laughter_dict, gt_laugh_segment, audio_length
                        )
                        tp_diff_start.append(tp_s)
                        tp_diff_end.append(tp_e)
                    else:
                        FN += 1
                        detection_eval.append(0)
                        detection_gt.append(1)
                for _ in range(int(len(gt_laughter_dict) * neg_sample_scale)):
                    ...
                    if (
                        np.sum(gt_laughter[int(start_time * hz) : int(end_time * hz)])
                        >= 1
                    ):
                        continue
                    if (
                        np.sum(eval_laughter[int(start_time * hz) : int(end_time * hz)])
                        >= 1
                    ):
                        FP += 1
                        detection_eval.append(1)
                ...
                if np.sum(gt_laughter[:]) >= 1:
                    ...
                    if np.sum(eval_laughter[:]) >= 1:
                        ...
                        if model_name == "Gillick2021":
                            ...
                            if basename not in _debug_gallick_samples:
                                _debug_ours_samples.add(basename)
                    else:
                        TN += 1
                        detection_eval.append(0)
...
{TP + FN},{FP + TN}
        ...
        for metric in metrices:
            res = evaluate_with_conf_int(
                np.array(detection_eval),
                metric,
                np.array(detection_gt),
                conditions=None,
                num_bootstraps=len(detection_gt),
                alpha=5,
            )
            # print(res)
            print(
                f"    {metric.__name__}: {r(res[0])}±{(r(res[1][1] - res[1][0]) / 2)}"
            )
        metrices = ["accuracy", "precision", "recall", "f1"]
        print("Segmentation metrices:")
        for metric in metrices:
            res = evaluate_with_conf_int(
                np.array(frame_wise_metrics[metric]),
                mean,
                None,
                conditions=None,
                num_bootstraps=len(frame_wise_metrics[metric]),
                alpha=5,
            )
            # print(r(res)
        ...
        mae_start = evaluate_with_conf_int(
            np.array(tp_diff_start),
            mean_absolute_error,
            np.array([0] * len(tp_diff_start)),
            conditions=None,
            num_bootstraps=len(tp_diff_start),
            alpha=5,
        )
        print(
            f"Start MAE: {r(mae_start[0])}±{(r(mae_start[1][1] - mae_start[1][0]) / 2)}"
        )
        mae_end = evaluate_with_conf_int(
            np.array(tp_diff_end),
            mean_absolute_error,
            np.array([0] * len(tp_diff_end)),
            conditions=None,
            num_bootstraps=len(tp_diff_end),
            alpha=5,
        )
