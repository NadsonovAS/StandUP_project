...
from laughter_segmentation.evaluation._utils.utils import concat_close, remove_short
from laughter_segmentation.train.model import Model
        ...
        for event in event_list.values():
            if not merged_events:
                ...
                for merged_event in merged_events.values():
                    if (
                        event["start_sec"] <= merged_event["end_sec"]
                        and event["end_sec"] >= merged_event["start_sec"]
                    ):
                        # Events overlap, merge them
                        merged_event["start_sec"] = min(
                            event["start_sec"], merged_event["start_sec"]
                        )
                        merged_event["end_sec"] = max(
                            event["end_sec"], merged_event["end_sec"]
                        )
                        merged = True
                        has_merged = True
                        # break
...
def bandpass(
    x,
    samplerate,
    fp=np.array([1000, 3000]),
    fs=np.array([1000, 3000]),
    gpass=3,
    gstop=40,
):
    ...
    N, Wn = signal.buttord(
        wp, ws, gpass, gstop
    )  # calculate the order and normalized frequency of the Butterworth
    b, a = signal.butter(
        N, Wn, "band"
...
def main(audio_path, output_dir, model_path, input_sec=7, batch_size=10):
    audio_model_name = "internalhell/wav2vec2-large-ru-5ep"
    ...
    with torch.no_grad():
        ...
        for array_idx in range(
            0, len(audio_array), int(sr * (input_sec - over_lap_sec)) * batch_size
        ):
            batched_arrays = []
            should_break = False
            for batch_idx in range(batch_size):
                array = audio_array[
                    array_idx
                    + batch_idx * int(sr * (input_sec - over_lap_sec)) : array_idx
                    + batch_idx * int(sr * (input_sec - over_lap_sec))
                    + sr * input_sec
                ]
            ...
            input_values = torch.from_numpy(np.array(batched_arrays)).type(
                torch.FloatTensor
            )
                    ...
                    if frame == 1:
                        ...
                        if status == "laughing" and idx == frame_count - 1:
                            laughter[str(laughter_idx)] = {
                                "start_sec": batch_start_sec
                                + (input_sec / frame_count) * start_idx,
                        ...
                        if status == "laughing":
                            ...
                            if start_idx == 0:
                                laughter[str(laughter_idx)] = {
                                    "start_sec": batch_start_sec
                                    + (input_sec / frame_count) * start_idx,
                                    "end_sec": batch_start_sec
                                    + (input_sec / frame_count) * end_idx,
                                }
                            else:
                                laughter[str(laughter_idx)] = {
                                    "start_sec": batch_start_sec
...
if __name__ == "__main__":
    ...
    if model_path is None:
        # If model_path is not provided, use the default path relative to the script
        script_dir = os.path.dirname(os.path.abspath(__file__))
        model_path = os.path.join(script_dir, "models", "model.safetensors")

    main(args.audio_path, args.output_dir, model_path)
