...
class Dataset:
    def __init__(self, lang, valid_count=None, streaming=False):
        self.encoded_dataset = datasets.load_dataset(
            f"../train/data/data_{lang}.py", streaming=streaming
        )

        if valid_count:
            self.encoded_dataset["validation"] = self.encoded_dataset[
                "validation"
            ].select(range(min(valid_count, len(self.encoded_dataset["validation"]))))
...
class CustomDataCollator:
    ...
    def __init__(self, input_sec, audio_model, debug):
        ...
        self.laughter_audio_dirs = [
            r"D:\datasets\vocals\vs_release_16k\audio_16k",
            r"D:\datasets\laughterscape_ver1.0\ver1.0\denoised",
            r"D:\datasets\manually_collected\laughter",
        ]
        self.laughter_audio_probs = [
            0.5,
            0.45,
            0.025,
            0.025,
        ]  # last one is for synthetic crowd laughters
        ...
        self.music_fragment_dir = (
            r"D:\datasets\IRMAS\IRMAS-TrainingData\IRMAS-TrainingData"
        )
        self.IRMAS_music_dirs = [
            r"D:\datasets\IRMAS\IRMAS-TestingData-Part1\IRMAS-TestingData-Part1\Part1",
        ...
        train_3 = (
            glob.glob(self.laughter_audio_dirs[2] + "/*.mp3")
            + glob.glob(self.laughter_audio_dirs[2] + "/*.wav")
            + glob.glob(self.laughter_audio_dirs[2] + "/*/*.mp3")
            + glob.glob(self.laughter_audio_dirs[2] + "/*/*.wav")
        )  # crowd laughters
        TRAIN_RATIO = 0.98
        self.train_audios = [
            train_1[: int(len(train_1) * TRAIN_RATIO)],
            train_2[: int(len(train_2) * TRAIN_RATIO)],
            train_3,
        ]
        self.eval_audios = [
            train_1[int(len(train_1) * TRAIN_RATIO) :],
        ...
        self.resample_to = [
            ...
            if int(
                librosa.load(
                    random.choice(self.train_audios[0]), sr=None, mono=True, duration=1
                )[1]
            )
            == self.sr
            else self.sr,
            ...
            if int(
                librosa.load(
                    random.choice(self.train_audios[1]), sr=None, mono=True, duration=1
                )[1]
            )
            == self.sr
            else self.sr,
        ...
        assert len(self.eval_audios[0]) and len(self.eval_audios[1]), (
            "eval audio not found in some dir"
        )
        ...
        self.train_base_audios = self.base_audios[
            : int(len(self.base_audios) * TRAIN_RATIO)
        ]
        self.eval_base_audios = self.base_audios[
            int(len(self.base_audios) * TRAIN_RATIO) :
        ]
        self.train_base_audios2 = self.base_audios2[
            : int(len(self.base_audios2) * TRAIN_RATIO)
        ]
        self.eval_base_audios2 = self.base_audios2[
            int(len(self.base_audios2) * TRAIN_RATIO) :
        ]
        ...
        for music_dir in self.IRMAS_music_dirs:
            ...
            audio_paths = [
                os.path.splitext(audio_path)[0].rsplit("-", 1)[0]
            ...
            for audio_path in audio_paths:
                ...
                self.music_audios += random.sample(
                    music_paths, min(3, len(music_paths))
                )
        ...
        if audio_model:
            ...
            print(
                "audio max length",
                int(self.feature_extractor.sampling_rate * self.input_sec),
            )
    ...
    def is_inappropriate_audio(self, array):
        ...
        if (
            max(array) < 0.1
        ):  # remove slient audio. should check min too but looks the same as -max
    ...
    def add_laughter_like_noise(self, base_audio, base_len):
        ...
        if (
            random.random() < 1 / 6
        ):  # choose from [No noise, Sigh, Cough, ThroatClearing, Sneeze, Sniff]
            return base_audio
        ...
        noise_array *= random.uniform(0.1, 1)
        base_audio[max(0, start_idx) : min(base_len, noise_len + start_idx)] += (
            noise_array[max(-start_idx, 0) : min(noise_len, base_len - start_idx)]
        )
        return base_audio
                ...
                if idx - start_frame_idx > no_laugh_part_duration:
                    if random.random() < 0.75:
                        self.debug and print(
                            f"add_sound_to_no_laugh_part {start_frame_idx}~{idx}"
                        )
                        margin = random.randint(
                            no_laugh_part_duration, idx - start_frame_idx
                        )
                        start_idx = random.randint(start_frame_idx, idx - margin)
                        base_audio = self.add_short_audio_fragment_to_specific_pos(
                            self.misc_audios,
                            base_audio,
                            base_len,
                            start_idx,
    ...
    def add_short_audio_fragment_to_specific_pos(
        self, audios, base_audio, base_len, start_idx, margin, vol
    ):
        ...
        fade_len = min(
            music_len // 2, int(self.sr * random.uniform(0.01, 0.5))
        )  # 0~0.5秒
        # if 0 < start_idx:
        ...
        fade_intensity = random.uniform(0.01, 0.95)
    ...
    def add_short_music_fragment(self, base_audio, base_len):
        for _ in range(1):
            ...
            music_array *= random.uniform(0.2, 1)
            # add fade in/out if start/end is in base_audio
            fade_len = min(
                music_len // 2, int(self.sr * random.uniform(0.01, 0.5))
            )  # 0~0.5秒
            ...
            # fade base_audio where music exists
            fade_intensity = random.uniform(0.01, 0.95)
            base_audio[max(0, start_idx) : min(base_len, music_len + start_idx)] *= (
                fade_intensity
            )

            base_audio[max(0, start_idx) : min(base_len, music_len + start_idx)] += (
                music_array[max(-start_idx, 0) : min(music_len, base_len - start_idx)]
            )
        return base_audio
    ...
    def add_short_misc_sound(self, base_audio, base_len):
        ...
        audio_array *= random.uniform(0.2, 1)

        base_audio[max(0, start_idx) : min(base_len, audio_len + start_idx)] += (
            audio_array[max(-start_idx, 0) : min(audio_len, base_len - start_idx)]
        )
        return base_audio

    def add_music(self, base_audio, base_len):
        music_audio_path = random.choice(self.music_audios)
        music_array = librosa.load(
            music_audio_path, sr=self.sr, mono=True, duration=20
        )[0]
        music_len = len(music_array)
        # decide random start point if music is longer than input_sec
        input_len = self.sr * self.input_sec
        music_array *= random.uniform(0.7, 1)
        ...
        board = pedalboard.Pedalboard(
            [
                pedalboard.Compressor(
                    threshold_db=-30, ratio=ratio, attack_ms=1.0, release_ms=100
                )
            ]
        )
    ...
    def custom_amplituder_small_portion(self, array, sr=16000, mul_fac=5):
        ...
        silent_section = detect_silence(
            dub_audio, min_silence_len=270, silence_thresh=-35
        )

        sr_mul = sr // 1000
        for sec in silent_section:
            fade_len = int(sr * 0.15)  # 0.15秒
            if (sec[1] - sec[0]) * sr_mul > (fade_len * 2):
                array[sec[0] * sr_mul : sec[0] * sr_mul + fade_len] *= np.linspace(
                    1, mul_fac, fade_len
                )
                array[sec[0] * sr_mul + fade_len : sec[1] * sr_mul - fade_len] *= (
                    mul_fac
                )
                if sec[1] * sr_mul < len(array):
                    array[sec[1] * sr_mul - fade_len : sec[1] * sr_mul] *= np.linspace(
                        mul_fac, 1, fade_len
                    )
            else:
    ...
    def reverb(
        ...
        if prob < 0.5:
            ...
            if wet_level is None:
                wet_level = random.uniform(0.0, 0.5)
            dry_level = 1
            width = 1.0
            freeze_mode = 0.0
            board = pedalboard.Pedalboard(
            ...
            if min_absorption_value is None:
    ...
    def crop_nonsilent_section(self, array):
        ...
        nonsilent_section = detect_nonsilent(
            dub_audio, min_silence_len=270, silence_thresh=-35
        )
        ...
        for nons_sec in nonsilent_section:
            start_idx, end_idx = (
                int(nons_sec[0] * self.sr_mul),
                int(nons_sec[1] * self.sr_mul),
            )
    ...
    def load_laugh(self, chosen_dataset, split):
        ...
        while trial_count < self.MAX_TRY:
            ...
            audio_path = random.choice(
                self.train_audios[chosen_dataset]
                if split == "train"
                else self.eval_audios[1 if chosen_dataset >= 2 else chosen_dataset]
            )
            # max duraion is 10 seconds but usually shorter
            laugh_audio = librosa.load(
                audio_path, sr=self.resample_to[chosen_dataset], mono=True, duration=10
            )[0]
            if random.random() < 0.05:
                try:
                    laugh_audio = self.reverb(laugh_audio)
                    self.debug and print(" reverb to laugh")
                except:
                    pass
            ...
            if random.random() < 0.05:
                try:
                    ...
                    laugh_audio = self.low_pass_filter(
                        laugh_audio, cutoff_freq=cutoff_freq
                    )
    ...
    def change_pitch_and_speed(self, array):
        ...
        if random.random() < 0.5:
            ...
            pitch_c = pedalboard.Pedalboard([pedalboard.PitchShift(semitones=pitch)])
            array = pitch_c(array, sample_rate=self.sr)
        if random.random() < 0.5:
            speed = min(max(0.75, random.gauss(1, 0.3)), 2)
            array = pedalboard.time_stretch(
                array, samplerate=self.sr, stretch_factor=speed
            )[0]
        return array
    ...
    def make_crowd_laugh(self, split):
        ...
        while trial_count < self.MAX_TRY:
            ...
            for _ in range(num_of_laughing_people):
                ...
                laugh_audio = self.low_pass_filter(
                    laugh_audio, cutoff_freq=low_pass_cutoff
                )
                start_idx = max(
                    0, int(random.gauss(default_start_idx, default_start_idx // 5))
                )
                end_idx = min(start_idx + len(laugh_audio), len(crowd_laugh_audio))
                crowd_laugh_audio[start_idx:end_idx] += laugh_audio[
                    : min(len(crowd_laugh_audio) - start_idx, len(laugh_audio))
                ]
            ...
            if len(crowd_laugh_audio) > self.sr * 5:
                crowd_laugh_audio = crowd_laugh_audio[
                    : int(self.sr * random.uniform(3.0, 6.0))
                ]
            fade_len = int(self.sr * random.uniform(0.5, 1.5))
            if fade_len < len(crowd_laugh_audio):
                crowd_laugh_audio[len(crowd_laugh_audio) - fade_len :] *= np.linspace(
                    1, 0, fade_len
                )
    ...
    def get_laugh_sample(self, base_audio, label, laugh_audio, base_len, weak_fade):
        ...
        if base_len < insert_idx + fade_start_offset:
            fade_start_offset = 0
        if insert_idx > fade_len:
            base_audio[
                insert_idx - fade_len + fade_start_offset : insert_idx
                + fade_start_offset
            ] *= np.linspace(1, fade_intensity, fade_len)
        base_audio[
            max(0, insert_idx + fade_start_offset) : min(
                insert_idx + laugh_len, base_len
            )
        ] *= fade_intensity
        if insert_idx + laugh_len < base_len - fade_len:
            base_audio[insert_idx + laugh_len : insert_idx + laugh_len + fade_len] *= (
                np.linspace(fade_intensity, 1, fade_len)
            )

        # change volume of laugh_audio randomly
        laugh_audio *= random.uniform(0.3, 1.2)
        # prevent laughter from becoming inaudible
        laugh_avg = np.mean(
            abs(
                laugh_audio[max(-insert_idx, 0) : min(laugh_len, base_len - insert_idx)]
            )
        )
        base_avg = np.mean(
            abs(base_audio[max(0, insert_idx) : min(base_len, laugh_len + insert_idx)])
        )
        if laugh_avg < base_avg:
            laugh_audio *= base_avg / laugh_avg  # *1.1

        base_audio[max(0, insert_idx) : min(base_len, laugh_len + insert_idx)] += (
            laugh_audio[max(-insert_idx, 0) : min(laugh_len, base_len - insert_idx)]
        )
    ...
    def getitem(self, split, laugh_count=None):
        ...
        base_audio = get_base_audio(
            (
                [self.train_base_audios, self.train_base_audios2][chosen_base_idx]
                if split == "train"
                else [self.eval_base_audios, self.eval_base_audios2][chosen_base_idx]
            ),
            self.sr,
            self.input_sec,
            [self.base_audio_dir, self.base_audio_dir2][
                chosen_base_idx
            ],  # to detect base_audio dataset
            (
                min(
                    1,
                    (self.current_iteration)
                    * 0.85
                    / self.curriculum_learning_100_percent_iteration,
                )
        ...
        laugh_count = (
            random.randint(0, self.MAX_LAUGH_COUNT)
            if laugh_count is None
            else laugh_count
        )
        ...
        if self.curriculum_learning and split == "train":
            base_audio *= min(
                1,
                self.current_iteration / self.curriculum_learning_100_percent_iteration,
            )
        ...
        while curr_laugh_count < laugh_count:
            ...
            chosen_dataset = random.choices(
                [0, 1, 2, 3], weights=self.laughter_audio_probs
            )[0]
            if chosen_dataset == 3:
            ...
            base_audio, label = self.get_laugh_sample(
                base_audio,
                label,
                laugh_audio,
                base_len,
        ...
        base_audio = self.add_sound_to_no_laugh_part(
            base_audio,
            base_len,
            label,
            min(
        ...
        if random.random() < 0.005:
            ...
            self.debug and print(
                f"shorten audio to {random_pos} and padding with 0 to the end"
            )
            ...
            assert len(base_audio) == len(label) == base_len, (
                f"{random_pos=}, {len(base_audio)=}, {len(label)=}, {base_len=}"
            )

        if not (
            split == "train"
            and self.curriculum_learning
            and self.current_iteration
            < int(self.curriculum_learning_100_percent_iteration * 0.75)
        ):
