...
class DiceLoss(nn.Module):
    ...
    def forward(self, inputs, targets, smooth=0.00001):
        ...
        dice = (2.0 * intersection) / (inputs.sum(dim=1) + targets.sum(dim=1) + smooth)
...
class Model(torch.nn.Module):
    def __init__(self, audio_model_name: str, device: str, sr: int):
        ...
        if "wav2vec" in audio_model_name:
            self.audio_model = Wav2Vec2ForAudioFrameClassification.from_pretrained(
                audio_model_name,
                num_labels=1,
    ...
    def kill_subprocess_randomly(self):
        ...
        children = [
            ...
            if child.io_counters()[2]
            == self.subprocesses_read_bytes_by_pids.get(child.pid, 0)
        ]
    ...
    def forward(
        ...
        **kwargs,
        ...
        if labels is not None:
            ...
            loss = loss_func(logits, labels.to(self.device)) + bce_loss_func(
    ...
    def beat_loss(self, array, gt, eval):
        ...
        fp_frames = fp_frames.repeat_interleave(round(array.shape[1] / 349), dim=1)[
            :, : array.shape[1]
        ]
        array *= fp_frames
        ...
        onset_envelope = librosa.onset.onset_strength(
            y=array.to("cpu").detach().numpy().copy(), sr=self.sr, hop_length=hop_length
        )
        ...
        for i in range(array.shape[0]):
            peaks, _ = find_peaks(onset_envelope[i], height=0.15)
