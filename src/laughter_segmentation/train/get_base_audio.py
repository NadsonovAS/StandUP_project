...
laugh_threshold = 0.005
max_laugh_threshold = 0.05  # 5%
speech_threshold = 0.01  # 1%
...
def get_base_audio(
    audios, sr, duration, base_audio_dir="", curriculum_learning_rate=1.0, debug=False
):
    if "audioset" in base_audio_dir:
        ...
        while trial < MAX_TRY_FILES:
            ...
            audio_array = librosa.load(audio_path, sr=sr, duration=duration, mono=True)[
                0
            ]
            if len(audio_array) < (duration * 0.97) * sr:
        ...
        while trial < MAX_TRY_FILES:
            ...
            with h5py.File(yamnet_path, "r") as f:
                ...
                laughter_probs = (
                    scores[:, LAUGHTER_IDX]
                    + scores[:, BABY_LAUGHER_IDX]
                    + scores[:, GIGGLE_IDX]
                    + scores[:, SNICKER_IDX]
                    + scores[:, BELLY_LAUGH_IDX]
                    + scores[:, CHUCKLE_IDX]
                )
                assert len(laughter_probs) > int(duration / interval), (
                    f"{audio_path=} is too short"
                )

                inner_trial = 0
                while inner_trial < MAX_TRY_IN_FILE:
                    start_idx = random.randint(
                        0, len(laughter_probs) - int((duration + 1) / interval)
                    )
                    ...
                    curr_laugh_threshold = laugh_threshold
                    if (
                        laughter_probs[start_idx : start_idx + int(duration / interval)]
                        < curr_laugh_threshold
                    ).all():  # and\
                        ...
                        debug and print(
                            f"non-laugh audio: {os.path.basename(audio_path)}, start_time: {start_idx * interval}"
                        )
                        return ar
                    else:
                        inner_trial += 1
